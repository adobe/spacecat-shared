...
- **`unzipRepositoryFromFile(zipFilePath, { maxEntries?, headroomFactor? })`** – Extracts a ZIP **from a file path**, streaming from disk (low, size-independent memory). Reads the central directory first and rejects archives that exceed `maxEntries` (default 100000), or whose extracted footprint — the on-disk archive size plus its uncompressed contents plus a per-entry filesystem-block cushion (each extracted file occupies at least one full block) — would not fit `/tmp`. Returns the extraction path. Prefer this over `unzipRepository(Buffer)` for large archives.
- **`assertTmpSpace(requiredBytes, { headroomFactor? })`** – Throws unless `/tmp` has room for `requiredBytes * headroomFactor` (default 1.25), based on usable (`bavail`) space.
