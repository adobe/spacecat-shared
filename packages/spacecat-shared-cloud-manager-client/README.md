...
- **`unzipRepositoryFromFile(zipFilePath, { maxEntries?, headroomFactor? })`** – Extracts a ZIP **from a file path**, streaming from disk (low, size-independent memory). Reads the central directory first and rejects archives whose uncompressed size would not fit `/tmp` or that exceed `maxEntries` (default 100000). Returns the extraction path. Prefer this over `unzipRepository(Buffer)` for large archives.
