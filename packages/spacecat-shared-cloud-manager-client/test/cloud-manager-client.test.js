...
describe('CloudManagerClient', () => {
  ...
  describe('zipRepository', () => {
    ...
    it('returns zip buffer when clone path exists', async () => {
      ...
      // Should call archiveFolder with followSymlinks: false (buffer-returning overload)
    ...
    it('logs a warning for broken symlinks but proceeds with zip', async () => {
      ...
      readlinkSyncStub.withArgs(path.join(enabledFarmsPath, 'broken.farm')).returns(brokenTarget);
      ...
      expect(Buffer.isBuffer(result)).to.be.true;
      expect(archiveFolderStub).to.have.been.calledOnce;
      // Broken symlink triggers a warn log during validateSymlinks
      expect(ctx.log.warn).to.have.been.calledOnce;
      expect(ctx.log.warn.firstCall.args[0]).to.match(/Broken symlink.*broken\.farm.*missing\.farm/);
    });
