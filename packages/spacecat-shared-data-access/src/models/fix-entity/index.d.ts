...
export interface FixEntityCollection extends BaseCollection<FixEntity> {
  ...
  rollbackFixWithSuggestionUpdates(fixEntityId: string, opportunityId: string, suggestionUpdates: Array<{suggestionId: string, opportunityId: string}>, options?: {token?: string}): Promise<{canceled: boolean, data: any[], updatedCount: number}>;
}
