...
describe('BaseCollection', () => {
  ...
  describe('S2S capability enforcement', () => {
    it('allows createMany when s2sCtx is empty (end-user)', async () => {
      ...
      await expect(instance.createMany(null))
        .to.be.rejectedWith('Failed to create many [mockEntityModel]: items must be a non-empty array');
    });

    it('allows createMany when S2S consumer has write capability', async () => {
      mockEntityRegistry.s2sCtx = {
        clientId: 'test-client',
        capabilities: ['mockEntityModel:write'],
        scopedOrgId: 'org1',
      };
      const instance = createInstance(
        mockElectroService,
    ...
    it('blocks createMany when S2S consumer lacks write capability', async () => {
      mockEntityRegistry.s2sCtx = {
        clientId: 'test-client',
        capabilities: ['mockEntityModel:read'],
        scopedOrgId: 'org1',
      };
      const instance = createInstance(
        mockElectroService,
      ...
      await expect(instance.createMany([{ someKey: 'value' }]))
        .to.be.rejectedWith('Forbidden: S2S consumer is missing required capability: mockEntityModel:write');
    });
  });
});
