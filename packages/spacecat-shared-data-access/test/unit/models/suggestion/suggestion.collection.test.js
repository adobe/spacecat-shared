...
describe('SuggestionCollection', () => {
  ...
  describe('grantSuggestion', () => {
    ...
    it('grants suggestions when tokens are available and RPC succeeds', async () => {
      ...
      const result = await instance.grantSuggestion(suggestionIds, siteId, tokenType);
      ...
      expect(mockElectroService.rpc).to.have.been.calledOnceWith(
        'grant_suggestion_consume_token',
        {
    ...
    it('throws DataAccessError when RPC returns an error', async () => {
      ...
      await expect(instance.grantSuggestion(suggestionIds, siteId, tokenType))
        .to.be.rejectedWith(DataAccessError, 'Failed to grant suggestions (grant_suggestion_consume_token)');
      expect(mockLogger.error).to.have.been.calledWith('grantSuggestion: RPC failed', rpcError);
    });
    ...
    it('throws DataAccessError when suggestionIds is not an array', async () => {
      await expect(instance.grantSuggestion('not-an-array', siteId, tokenType))
    ...
    it('throws DataAccessError when suggestionIds contains empty strings', async () => {
      await expect(instance.grantSuggestion(['sug-1', ''], siteId, tokenType))
        .to.be.rejectedWith(DataAccessError, 'suggestionIds must be an array of non-empty strings');
    });

    it('throws DataAccessError when siteId is missing', async () => {
      await expect(instance.grantSuggestion(suggestionIds, '', tokenType))
        .to.be.rejectedWith(DataAccessError, 'siteId is required');
    });

    it('throws DataAccessError when tokenType is missing', async () => {
      await expect(instance.grantSuggestion(suggestionIds, siteId, ''))
