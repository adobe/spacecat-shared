...
describe('SuggestionModel', () => {
  ...
  describe('getData and setData', () => {
    ...
    it('strips a synthetic factId (matching suggestionKey) before persisting, on a getData -> merge -> setData round trip', () => {
      mockRecord.suggestionKey = 'site:site1:backlink:abc:referrer:def';
      ({ model: instance } = createElectroMocks(Suggestion, mockRecord));

      const data = instance.getData();
      expect(data.factId).to.equal('site:site1:backlink:abc:referrer:def');

      instance.setData({ ...data, extra: 'field' });

      expect(instance.record.data).to.deep.equal({ info: 'sample data', extra: 'field' });
      expect(instance.record.data).to.not.have.property('factId');
    });

    it('persists a real factId that differs from suggestionKey', () => {
      mockRecord.suggestionKey = 'site:site1:backlink:abc:referrer:def';
      ({ model: instance } = createElectroMocks(Suggestion, mockRecord));

      instance.setData({ info: 'sample data', factId: 'legacy:op1:sug1' });

      expect(instance.record.data).to.deep.equal({ info: 'sample data', factId: 'legacy:op1:sug1' });
    });

    it('persists normally when there is no suggestionKey at all', () => {
      instance.setData({ info: 'sample data', factId: 'legacy:op1:sug1' });
  ...
  describe('toJSON factId backfill', () => {
    it('backfills data.factId from suggestionKey when data has no factId', () => {
      mockRecord.suggestionKey = 'site:site1:backlink:abc:referrer:def';
      ({ model: instance } = createElectroMocks(Suggestion, mockRecord));

      expect(instance.toJSON().data).to.deep.equal({
        info: 'sample data',
        factId: 'site:site1:backlink:abc:referrer:def',
      });
    });
    ...
    it('leaves data unchanged when neither factId nor suggestionKey is present', () => {
      expect(instance.toJSON().data).to.deep.equal({ info: 'sample data' });
    });
  });
