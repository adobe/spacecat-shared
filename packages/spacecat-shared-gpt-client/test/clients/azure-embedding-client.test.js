...
describe('AzureEmbeddingClient', () => {
  ...
  describe('createEmbeddings', function () {
    ...
    it('throws when more embeddings are returned than inputs', async () => {
      nock(mockContext.env.AZURE_EMBEDDING_ENDPOINT)
        .post(path)
        .query({ 'api-version': '2024-02-01' })
        .reply(200, { data: [{ index: 0, embedding: [0.1] }, { index: 1, embedding: [0.2] }] });

      await expect(client.createEmbeddings(['x']))
        .to.be.rejectedWith('Invalid response format.');
    });

    it('throws when a data item is missing its index', async () => {
      nock(mockContext.env.AZURE_EMBEDDING_ENDPOINT)
        .post(path)
        .query({ 'api-version': '2024-02-01' })
        .reply(200, { data: [{ embedding: [0.1] }] }); // no `index`
  ...
  describe('retry on transient failures', function () {
    ...
    it('caps an oversized Retry-After at retryMaxDelayMs', async () => {
      // Retry-After 3600s would be an hour; the cap bounds the actual sleep to retryMaxDelayMs.
      nock(endpoint).post(path).query({ 'api-version': '2024-02-01' })
      ...
      // Assert the computed delay directly (deterministic, no wall-clock): the retry log has it.
      const retryLog = mockLog.info.getCalls()
        .map((c) => c.args[0])
        .find((m) => m.includes('retry'));
      expect(retryLog).to.match(/in 5ms$/);
    });
