...
// Match exact Google Maps runtime classes only, never customer wrappers by name or substring.
// Preserve open InfoWindow headers/bodies, but not marker labels, overlays, or custom controls.
...
const MAPS_CONTROL_SELECTORS = [
  ...
  // LGLeeN is generated by Google and may change in a future Maps release.
  ...
  outermostMapRoots.forEach((mapRoot) => {
    mapRoot.querySelectorAll('.gm-style-iw').forEach((infoWindow) => {
      ...
      preservedNodes.forEach((node) => {
        mapRoot.parentNode.insertBefore(node, mapRoot);
      });
    });
    mapRoot.remove();
  });
...
function removeMapWidgetsCheerio($) {
  ...
  outermostMapRoots.forEach((mapRoot) => {
    ...
    $mapRoot.find('.gm-style-iw').each((j, infoWindow) => {
      ...
      const $contentParts = $infoWindow.find('.gm-style-iw-ch, .gm-style-iw-d');
      const $preservedNodes = $contentParts.length ? $contentParts : $infoWindow;

      $preservedNodes.each((k, node) => {
        $mapRoot.before(node);
      });
    });
    $mapRoot.remove();
