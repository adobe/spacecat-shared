...
Authorship/timestamp metadata (`AIOPromptMetadata`: `created_at/by`, `updated_at/by`) is a
Semrush-vendored `/v3` prompts API using RFC 7396 JSON Merge Patch — real, spec-vendored paths
(not mock-owned), superseding the earlier `*-with-metadata` ADR guess this mock briefly modelled.

| Method + path | Behaviour |
| --- | --- |
| `POST /v3/workspaces/{id}/projects/{project_id}/aio/prompts` | create; body `{ items: [{ name, metadata? }], tag_ids? }` (shared `tag_ids`) → `201 { items: [{id,name,is_new,metadata}], existing_count }` — one result PER REQUEST ITEM; a DEDUPE hit (existing name) PRESERVES the stored metadata and reports `is_new: false`; every `tag_id` must resolve or the whole call `500`s (atomic, mirrors the v2 id-based create); an over-length `created_by`/`updated_by` (>100 chars, the live CHECK) `400`s the whole batch; **metered** on new items only |
| `POST /v3/workspaces/{id}/projects/{project_id}/aio/prompts/tagged` | create; body `{ prompts: [{ name, tags?: [tagName…], metadata? }] }` (PER-ITEM tag names) → same `201` shape as above; tag names mint ROOT tags when absent (mirrors v2 `tagged`); same dedupe/CHECK/quota rules |
| `PATCH /v3/workspaces/{id}/projects/{project_id}/aio/prompts/{prompt_id}` | combined edit; body `{ name?, metadata? }`, at least one required (`400` otherwise) → `204`; `metadata: null` WIPES the whole block, a `metadata` object RFC-7396-merges (absent key = keep, string = set, explicit `null` = delete a key); a `name` colliding with a SIBLING prompt's exact text → `409`, nothing mutated (same rule as `/rename`); an over-length author field → `400`, nothing mutated |
| `PATCH /v3/workspaces/{id}/projects/{project_id}/aio/prompts/{prompt_id}/metadata` | single-prompt metadata-only merge (body is `AIOPromptMetadataPatch` directly) → `204`; `404` unknown id; `400` on the author-length CHECK |
| `PATCH /v3/workspaces/{id}/projects/{project_id}/aio/prompts/metadata` | BATCH metadata merge; body `{ items: [{ prompt_id, metadata }] }`, ONE TRANSACTION → `204`; an unknown `prompt_id` OR any item violating the author-length CHECK rolls the WHOLE batch back (`404` / `400` respectively, nothing written for any item) |
