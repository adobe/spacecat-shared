...
| `POST /v2/workspaces/{id}/projects/{project_id}/aio/prompts/{prompt_id}/rename` | `renamePrompt` | IN-PLACE text edit (body `{ new_name }`) → `200 { id, name, is_updated }` with the SAME prompt id (rename never mints a new prompt); a `new_name` equal to ANOTHER prompt's text → `409` with nothing mutated (live-verified 2026-07-14, overlay CR17); a rename onto the prompt's OWN current text → `200` with `is_updated: false` (a no-op, not a conflict); unknown `prompt_id` → `404 { message: 'not found' }`. The rename is visible on BOTH the draft and default `by_tags` reads (the mock stores a single `name`; live keeps the old text in the live view until publish) |
