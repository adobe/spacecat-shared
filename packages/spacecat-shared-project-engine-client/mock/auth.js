...
/**
 * Bearer-auth modelling for the Project Engine mock.
 *
 ...
 * header AND a malformed `Bearer <garbage>` both 401; only a valid IMS token is let through). The
 * mock mirrors that at the gate: it requires `Authorization: Bearer <token>` to be PRESENT but —
 * being a test double, not an IMS verifier — does NOT validate the token's contents. Every real
 * API route guards on this via `context.authError($.headers)` (wired onto the {@link Context});
 * the test-harness control routes (`/__reset`, `/__seed`, `/__dump`, `/__quota`) are deliberately
 * exempt, since they are plumbing, not part of the emulated API surface.
 *
 * The 401 is returned as a plain `{ status, body, contentType }` so it bypasses Counterfact's
 * response-schema validation (401 is not declared per-operation in the spec) — the same raw-return
 * mechanism the control routes and the quota 405 use.
 */
