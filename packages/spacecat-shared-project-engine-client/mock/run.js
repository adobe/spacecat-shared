 ...
 * calls therefore has an explicit handler under `mock/counterfact/routes/**` — the stateful spine
 * (projects, ai_models, prompts, benchmarks, brand_urls) plus the catalog/echo reads (languages,
 * global ai_models, brand-topics, tags, ci competitors, publish, init_status). The shared store
 ...
 * the overlay corrections (e.g. the `createProjectTags` array, CR6) are what our handlers are
 * checked against. Counterfact honours a Swagger-2.0 `basePath` as a serving prefix but silently
...
const LIB_FILES = ['store.js', 'stateful.js', 'factories.js', 'quota.js', 'auth.js', 'seeds.js', 'context.js'];
