...
export default class ScrapeClient {
   ...
   * @returns {Promise<Response>} JSON representation of the scrape jobs
   */
  async getScrapeJobsByBaseURL(baseURL) {
    ...
    try {
      ...
      const jobs = await this.scrapeSupervisor.getScrapeJobsByBaseURL(decodedBaseURL);
      if (!jobs || jobs.length === 0) {
        return [];
      }
      return jobs.map((job) => ScrapeJobDto.toJSON(job));
    } catch (error) {
      const msgError = `Failed to fetch scrape jobs by baseURL: ${decodedBaseURL}, ${error.message}`;
      this.config.log.error(msgError);
      throw new Error(msgError);
    }
  }
