...
`RateLimitAwareHttpClient` is applied automatically by `TicketClientFactory`. It retries HTTP 429 responses up to 4 times, honouring the `Retry-After` header when present and falling back to exponential backoff (1 s, 2 s, 4 s, 8 s).
