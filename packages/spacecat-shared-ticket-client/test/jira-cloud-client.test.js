...
describe('JiraCloudClient', () => {
  ...
  describe('createTicket', () => {
    ...
    it('truncates summary at exactly 255 Unicode code points, not UTF-16 chars (surrogate safety)', async () => {
      const httpClient = makeHttpClient({ id: '1', key: 'ASO-1' });
      const client = new JiraCloudClient(
        VALID_CONFIG,
        makeCredentialManager(),
      ...
      // Each 𝌆 is a supplementary plane char = 2 UTF-16 code units.
      ...
      expect(codePoints).to.have.length(255);
      expect(codePoints[254]).to.equal('𝌆');
    });
    ...
    it('handles deeply nested blockquotes without crashing (DoS protection)', async () => {
      const httpClient = makeHttpClient({ id: '1', key: 'ASO-1' });
    ...
    it('truncates input over 50 KB and still produces valid ADF', async () => {
      const httpClient = makeHttpClient({ id: '1', key: 'ASO-1' });
      const client = new JiraCloudClient(VALID_CONFIG, makeCredentialManager(), httpClient, makeLog()); // eslint-disable-line max-len
      const massive = 'word '.repeat(15000); // ~75 KB
      await expect(
    ...
    it('pads empty fenced code block with space to satisfy ADF constraint', async () => {
      const httpClient = makeHttpClient({ id: '1', key: 'ASO-1' });
      const client = new JiraCloudClient(
        VALID_CONFIG,
        makeCredentialManager(),
      ...
      const node = body.fields.description.content[0];
      expect(node.type).to.equal('codeBlock');
      // Must not be empty — padded with space
      expect(node.content[0].text).to.equal(' ');
    });

    it('strips HTML tags from raw HTML block tokens (plain-text contract)', async () => {
      const httpClient = makeHttpClient({ id: '1', key: 'ASO-1' });
      const client = new JiraCloudClient(
        VALID_CONFIG,
        makeCredentialManager(),
      ...
      const body = JSON.parse(httpClient.fetch.firstCall.args[1].body);
      // HTML tags must be stripped from the ADF output; content becomes plain text
      const rawJson = JSON.stringify(body.fields.description);
      expect(rawJson).to.not.include('<script>');
    });
  });
