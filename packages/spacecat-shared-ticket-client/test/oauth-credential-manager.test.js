...
describe('OAuthCredentialManager', () => {
  ...
  describe('refreshAuthHeaders', () => {
    ...
    it('SM write all 3 fail and re-read still expired — writes requiresReauth and throws', async () => {
      const smClient = {
        getSecretValue: sinon.stub()
          .onFirstCall().resolves({ SecretString: JSON.stringify(EXPIRED_SECRET) })
          .onSecondCall()
        ...
        putSecretValue: sinon.stub()
          .onFirstCall().rejects(new Error('SM error'))
          .onSecondCall()
          .rejects(new Error('SM error'))
          .onCall(2)
          .rejects(new Error('SM error'))
          .onCall(3)
          .resolves({}), // #writeReauthFlag write
      };
      const httpClient = makeHttpClient({
        access_token: 'token',
      ...
      const reauthWrite = JSON.parse(smClient.putSecretValue.getCall(3).args[0].SecretString);
      expect(reauthWrite.requiresReauth).to.be.true;
    });
