...
describe('TierClient', () => {
  ...
  describe('createEntitlement', () => {
    ...
    it('should ignore an array quotaOverrides and use the default quotas', async () => {
      mockDataAccess.Entitlement.findByOrganizationIdAndProductCode.resolves(null);
      mockDataAccess.Entitlement.create.resolves(mockEntitlement);
      mockDataAccess.SiteEnrollment.create.resolves(mockSiteEnrollment);

      // `typeof [] === 'object'`, so without an Array.isArray guard the array's numeric
      // indices would spread in as `0`, `1`, ... keys and corrupt the persisted quotas.
      await tierClient.createEntitlement('PAID', [500]);
    ...
    it('should ignore a null quotaOverrides passed explicitly and use the default quotas', async () => {
      mockDataAccess.Entitlement.findByOrganizationIdAndProductCode.resolves(null);
      mockDataAccess.Entitlement.create.resolves(mockEntitlement);
      mockDataAccess.SiteEnrollment.create.resolves(mockSiteEnrollment);

      // Passing `null` explicitly bypasses the default parameter, so the falsy guard
      // (not the default `{}`) is what must keep the quotas at their defaults.
      await tierClient.createEntitlement('PAID', null);
