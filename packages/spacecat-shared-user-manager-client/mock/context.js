...
/**
 * The Counterfact per-path Context for the User Manager mock.
 *
 ...
 * `seed()` (`POST /__seed`), and `dump()` (`GET /__dump`) — so an E2E / the cross-repo harness can
 * drive and inspect mock state between cases. It also wires {@link createQuota} as `quota`
 * (parent-pool metering; pool set via `POST /__quota`, enforced on child create / resource transfer
 * — the 422 the live API returns for insufficient available units) and {@link authError} as
 * `authError` (bearer-auth gate; every real route guards on it, the `__*` control routes are exempt
 * — see mock/auth.js).
 *
 * Startup seed precedence: an explicit `seedFile` (a JSON {@link Snapshot} path, e.g. one the
 * harness generates from the same fixtures it loads into Postgres so the workspace ids line up)
 * wins over a named `seed`, which falls back to the default. `seedFile` is read with `node:fs` at
 * construction.
 *
...
export class Context {
  ...
  constructor({ seed, seedFile } = {}) {
    ...
    // Parent-pool metering over the same store (pools live in the `workspace_pool` collection, so
    // they ride along in seed / reset / dump; a create/transfer draws against the parent's pool).
