...
/**
 * Bearer-auth guard injection for the User Manager mock's materialized handlers.
 *
 ...
 * (reset/seed/dump/quota/status) are test-harness plumbing, not part of the emulated API, so they
 * are exempt. The guard returns a raw `{ status, body, contentType }`, which bypasses Counterfact
 * response validation (the gateway 401 body differs from the spec's error envelope) — the same
 * mechanism the quota 422 uses.
 *
