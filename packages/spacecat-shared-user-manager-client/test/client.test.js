...
import { createSerenityUserManagerApiClient } from '../src/client.js';
...
describe('createSerenityUserManagerApiClient', () => {
  it('sends the IMS token as Authorization: Bearer, not Auth-Data-Jwt', async () => {
    ...
    // the client appends the fixed /enterprise/users/api prefix to the supplied origin
    expect(request.url).to.equal('https://serenity.example/enterprise/users/api/v1/countries');
  });
  ...
  it('resolves an async token getter per request (no token captured at construction)', async () => {
    ...
    expect(fetch.firstCall.args[0].url)
      .to.equal('https://serenity.example/enterprise/users/api/v1/countries');
  });
  ...
  it('strips embedded credentials from the base URL — never forwards user:pass', async () => {
    const fetch = sandbox.stub().callsFake(() => Promise.resolve(json({ ok: true })));
    // Distinctive credential tokens that do NOT collide with the `/enterprise/users/api` prefix,
    // so the "credential stripped" assertions can't be satisfied by an accidental path substring.
    const client = createSerenityUserManagerApiClient({
      baseUrl: 'https://creduser:credpass@serenity.example/some/path',
      authToken: 'raw-ims-jwt',
      fetch,
    });
    ...
    expect(url).to.equal('https://serenity.example/enterprise/users/api/v1/countries');
    expect(url).to.not.contain('creduser');
    expect(url).to.not.contain('credpass');
  });
